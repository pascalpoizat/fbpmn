...
writeSToHTML :: FilePath -> Log -> IO ()
writeSToHTML p = writeFile p . toString . encodeSLogToHtml
...
genSSetup :: Log -> Text
genSSetup (Log _ _ Success _) =
  [text|
  <!-- success log, nothing to generate -->
  |]
genSSetup (Log _ _ Failure mcex) =
  case mcex of
    Nothing -> [text||] -- not possible
    Just cex -> foldMap genSForState cex
...
encodeSLogToHtml l =
  ...
  <html>
    ...
    <body>
      ...
      <div id="subs">
        <div id="title">Subs status</div>
        <div id="status"></div>
      </div>
      <div id="sigma">
        <div id="title">Sigma status</div>
        <div id="status"></div>
      </div>
      <script>
        ...
        function subsToJSON(subs) {
            return JSON.stringify([...subs]);
        }

        function sigmaToJSON(net) {
            return JSON.stringify([...sigma]);
        }
        ...
        function animate(registry,canvas,overlays,csteps,markings,prestep,step,nbsteps,pos) {
          ...
          $("#network #status").html(netToJSON(net)); 
          $("#subs #status").html(subsToJSON(subs))
          $("#sigma #status").html(sigmaToJSON(sigma))
          return markings;
        }
