...
BeforeAll {
    . fbpmn-private/scripts/wfbpmn-check.ps1
    function Replace {
        param($path)
        $OLD_PATH = $env:PATH
        $env:PATH = $env:PATH -replace ($path, ":")
        wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/e001ClientSupplier.bpmn 2
    ...
    function Compare-partially-JSON {
        ...
        for ($i = 0 ; $i -le (($json_observed.$model.psobject.properties.name).count - 1) ; $i++ ) {
            #for every network
            for ($j = 0 ; $j -le (($json_observed.$model.($Network[$i]).psobject.properties.name).count - 1) ; $j++) {
                #for every props
...
Describe 'fbpmn-check' {
    Context "Usage" {
        Context "workers" {
            It 'Good exit for wrong args.Length' {
                wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/e001ClientSupplier.bpmn 2 3 
                $LASTEXITCODE | Should -Be 1
            } 
            It 'No args -> workers = 1' {
                wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/e001ClientSupplier.bpmn 1 *> /tmp/e001ClientSupplier.out
            ...
            It 'Correct args.Length -> workers = args[1]' {
                wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/e001ClientSupplier.bpmn 3 *> /tmp/e001ClientSupplier.out
                $a = Select-String "Working in" -Path /tmp/e001ClientSupplier.out -NoEmphasis 
                $a -match "with (\d)"
        ...
        Context "no userfiles" {
            ## revoir ça
            It 'Good exit for no .userdefs' {
                wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/e001ClientSupplier.bpmn 2 3 
                $LASTEXITCODE | Should -Be 1
            } 
            It 'Good exit for no .userprops' {
                wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/e001ClientSupplier.bpmn 2 3 
                $LASTEXITCODE | Should -Be 1
            } 
            It 'Good exit for no .constraint' {
                wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/e001ClientSupplier.bpmn 2 3 
                $LASTEXITCODE | Should -Be 1
            } 
        }
    }
    Context "Compare partially observed with expected" {
        It "Differentiated JSON tests" { 
            ...
            foreach ($f in $f_bpmn) {
                ...
                wfbpmn-check.ps1 $fullpath *> $null
                if (-NOT(Compare-partially-JSON $model "$env:FBPMN_HOME/models/bpmn-origin/expected/$model.json" "$model.json" $true)) {
                    $r++                    
                } 
            }
            $r | Should -Be 0
    ...
    Context "Test-Path failed" {
        It "BPMN don't exists" {
            wfbpmn-check.ps1 $env:FBPMN_HOME/models/bpmn-origin/src/dontexist.bpmn 2  
            $LASTEXITCODE | Should -Be 1
        }
    }
}
