    ...
    <$> subparser
      ( command "version" (info (pure CVersion) (progDesc "prints the version"))
          ...
          <> command
            ...
            ( info
                parserSLog2Html
                (progDesc "transforms a TLA+ space log from LOG to HTML")
            )
      )
...
parserSLog2Html :: Parser Command
parserSLog2Html =
  CSLog2Html
    <$> argument
      str
...
run (Options (CSLog2Html pin pout)) = slog2html False pin pout
